...
"""
Implémentation de dGLMNET distribué multithread
...
def partial_solver(Xm, y, wm, lmbd):
    ...
    px = sigmoid(Xm @ wm)
    doublev = px*(1-px)
    z = (((y+1)/2) - px) / doublev #Attention : y is a problem for nogil threading because it is referenced here !
...
def dGLMNET_solver(X, y, lmbd, M=2, max_iter=100, tol=1e-6):
    ...
    Programmée de manière distribuée avec multithreads
    ...
    for i in range(max_iter):
        ...
        # Calculs parallélisés grâce au multithreading Python 
        with concurrent.futures.ThreadPoolExecutor(max_workers=M) as P:
            DeltaWms = P.map(map_function, Xms, wms)
        
        # Calcul de DeltaW
        m=0
        for DeltaWm in DeltaWms:
            DeltaW[S[m]] += DeltaWm
            m += 1

...
"""
Implémentation de dGLMNET de manière séquentielle sans parallélisation
...
def dGLMNET_solver(X, y, lmbd, M=2, max_iter=100, tol=1e-6):
    ...
    descente de coordonnées par blocs
    Programmée de manière séquentielle sans parallélisation
    ...
    for i in range(max_iter):
        ...
        DeltaWs = np.zeros((M,p))
        
        # Calculs séquentiels des DeltaW_m
        for m in range(M):
            Xm  = X[:, S[m]]
            DeltaWs[m, S[m]] = partial_solver(Xm, y, w[S[m]], lmbd)
        
        # Calcul de DeltaW
        DeltaW = DeltaWs.sum(axis = 0)
